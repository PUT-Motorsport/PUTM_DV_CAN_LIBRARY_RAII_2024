#pragma once
#include <cstdint>

/* Primary Can Bus */
const uint16_t DRIVER_INPUT_CAN_ID = 0x05;
const uint16_t PC_MAIN_CAN_ID = 0x10;
const uint16_t LAP_TIMER_CAN_ID = 0x11;
const uint16_t DASHBOARD_CAN_ID = 0x15;

const uint16_t REARBOX_SAFETY_CAN_ID = 0x25;
const uint16_t REARBOX_TEMPERATURE_CAN_ID = 0x26;
const uint16_t REARBOX_MISCELLANEOUS_CAN_ID = 0x27;

const uint16_t PDU_CHANNEL_CAN_ID = 0x30;
const uint16_t PDU_DATA_CAN_ID = 0x31;

const uint16_t FRONT_DATA_CAN_ID = 0x35;
const uint16_t BMS_HV_MAIN_CAN_ID = 0x45;
const uint16_t BMS_LV_MAIN_CAN_ID = 0x55;
const uint16_t BMS_LV_TEMPERATURE_CAN_ID = 0x56;
const uint16_t AMK_TEMP_DATA_ID = 0x60;


const uint16_t XSENS_ACCELERATION             = 0x70;
const uint16_t XSENS_ACCELERATION_HR          = 0x71;
const uint16_t XSENS_ALLTITUDE_ELLIPSOID      = 0x72;
const uint16_t XSENS_DELTA_Q                  = 0x73;
const uint16_t XSENS_ERROR                    = 0x74;
const uint16_t XSENS_EULER                    = 0x75;
const uint16_t XSENS_FREE_ACCELERATION        = 0x76;
const uint16_t XSENS_INERTIAL_DATA            = 0x77;
const uint16_t XSENS_MAGNETIC_FIELD           = 0x78;
const uint16_t XSENS_ORIENTATION              = 0x79;
const uint16_t XSENS_POSITION                 = 0x7A;
const uint16_t XSENS_RATE_OF_TURN             = 0x7B;
const uint16_t XSENS_RATE_OF_TURN_HR          = 0x7C;
const uint16_t XSENS_STATUS                   = 0x7D;
const uint16_t XSENS_TEMPERATURE_AND_PRESSURE = 0x7E;
const uint16_t XSENS_UTC                      = 0x7F;
const uint16_t XSENS_VELOCITY                 = 0x80;

/* Secondary Can Bus */
constexpr uint16_t FRONT_RIGHT_AMK_ACTUAL_VALUES_1_CAN_ID = 0x286; // 
constexpr uint16_t FRONT_RIGHT_AMK_ACTUAL_VALUES_2_CAN_ID = 0x287; // 
constexpr uint16_t FRONT_RIGHT_AMK_SETPOINTS_1_CAN_ID = 0x185;     // 

constexpr uint16_t FRONT_LEFT_AMK_ACTUAL_VALUES_1_CAN_ID = 0x288; //
constexpr uint16_t FRONT_LEFT_AMK_ACTUAL_VALUES_2_CAN_ID = 0x289; //
constexpr uint16_t FRONT_LEFT_AMK_SETPOINTS_1_CAN_ID = 0x186; // 

constexpr uint16_t REAR_RIGHT_AMK_ACTUAL_VALUES_1_CAN_ID = 0x284; // 
constexpr uint16_t REAR_RIGHT_AMK_ACTUAL_VALUES_2_CAN_ID = 0x285; // 
constexpr uint16_t REAR_RIGHT_AMK_SETPOINTS_1_CAN_ID = 0x184; // 

constexpr uint16_t REAR_LEFT_AMK_ACTUAL_VALUES_1_CAN_ID = 0x282; //
constexpr uint16_t REAR_LEFT_AMK_ACTUAL_VALUES_2_CAN_ID = 0x283; //
constexpr uint16_t REAR_LEFT_AMK_SETPOINTS_1_CAN_ID = 0x183; 
